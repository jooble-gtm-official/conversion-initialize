...

{
  ...
  "version": 1,
  "securityGroups": [],
  "displayName": "Jooble conversion initialize",
  ...
  "description": "Saves the fact that user comes from jooble",
  "containerContexts": [
    "WEB"
  ]
}
...
const conversionUrl = 'https://' + referrerHost + '/auction/api/conversion/js_pixel?utm_away=' + utmAway;

const jbl_setWithExpiry = (key, value, milisecondsToLive) => {
  const nowMillis = getTimestampMillis();

  // `item` is an object which contains the original value
  // as well as the time when it's supposed to expire
  const item = {
    value: value,
    expiry: nowMillis + milisecondsToLive,
  };
...
Created on 15.11.2021, 18:23:59
